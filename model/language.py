...
class Language_Obj:
    ...
    def get_json(self):
        return {self.languageName: { "ISO-Code": self.iso_code, "Family": self.languageFamily.value[1],"Branch":self.languageBranch.value[1]  if self.languageBranch else None,  "Speakers": self.languageSpeakers, "Countries": self.countriesSpoken,"Scripts": [s.value[1] for s in self.languageScripts]} }
